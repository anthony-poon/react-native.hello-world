export const COLOR_DEFAULT = "#212529";
export const COLOR_PRIMARY = "#007bff";
export const COLOR_SECONDARY = "#6c757d";
export const COLOR_SUCCESS = "#28a745";
export const COLOR_DANGER = "#dc3545";
export const COLOR_WARNING = "#ffc107";
export const COLOR_INFO = "#17a2b8";
export const COLOR_LIGHT = "#f8f9fa";
export const COLOR_DARK = "#343a40";
export const COLOR_MUTED = "#6c757d";
export const COLOR_WHITE = "#fff";
export const COLOR_BORDER = "#DEE2E6";

export const COLOR_PRIMARY_TINT_2 = "#8BC4FF";
export const COLOR_PRIMARY_TINT_1 = "#2B93FF";
export const COLOR_PRIMARY_SHADE_1 = "#014892";
export const COLOR_PRIMARY_SHADE_2 = "#01101F";